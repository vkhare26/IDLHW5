...
class DDIMScheduler(DDPMScheduler):    
    ...
    def step(
        ...
        pred_sample_direction = model_output / torch.sqrt(beta_prod_t) 

        # TODO: 6. compute x_t without "random noise" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf
        prev_sample = torch.sqrt(alpha_prod_t_prev) * sample + torch.sqrt(beta_prod_t_prev) * pred_sample_direction
        ...
        if eta > 0:
            ...
            prev_sample = variance_noise * (eta * np.sqrt(variance))
